...
export default function UsersView({ users, meId }: { users: Row[]; meId: string }) {
  ...
  return (
    <div>
      ...
      <table>
        <thead><tr><th scope="col">Handle</th><th scope="col">Email</th><th scope="col">Events</th><th scope="col">Conns</th><th scope="col">Actions</th></tr></thead>
        <tbody>
          {shown.map((u) => (
            <tr key={u.id}>
              <td>{u.handle}{u.ghost ? ' 👻' : ''}{u.id === meId ? ' (you)' : ''}</td>
              <td>{u.email ?? '—'}</td>
              <td>{u.events}</td>
              <td>{u.connections}</td>
              <td>
      ...
      {temp && <p role="alert">Temp password (shown once): <code>{temp.pw}</code> <button onClick={() => setTemp(null)}>Dismiss</button></p>}
    </div>
  );
}
