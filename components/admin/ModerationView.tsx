...
export default function ModerationView({ events, connections }: { events: Ev[]; connections: Conn[] }) {
  ...
  async function run(id: string, fn: () => Promise<void>, confirmMsg: string) {
    ...
    catch { setErr("Couldn't complete that action — it's enforced server-side; nothing changed."); }
  ...
  return (
    <div>
      ...
      <table>
        <thead><tr><th scope="col">Title</th><th scope="col">Type</th><th scope="col">Vis</th><th scope="col">Creator</th><th scope="col">Action</th></tr></thead>
        <tbody>{events.map((e) => (
          <tr key={e.id}><td>{e.title}</td><td>{e.type}</td><td>{e.visibility}</td><td>{e.creatorHandle ?? '—'}</td>
            <td><button disabled={busy === e.id} onClick={() => run(e.id, () => adminDeleteEvent(e.id), `Delete "${e.title}"? This cannot be undone.`)}>Delete</button></td></tr>
        ))}</tbody>
      </table>
      <h2>Connections ({connections.length})</h2>
      <table>
        <thead><tr><th scope="col">A</th><th scope="col">B</th><th scope="col">Status</th><th scope="col">Action</th></tr></thead>
