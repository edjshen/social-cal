  ...
  return (
    <>
      <div className="topbar">
        <div><div className="kicker">Plans</div><div className="h-title">What you're in</div></div>
        ...
        return (
          <div key={ev.id}>
            ...
            <div className="card">
              ...
              <div className="meta">
                {ev.location || ''}
                {ev.attendeeCount ? <><span className="dot" />{ev.attendeeCount} in</> : null}
              </div>
              <div className="row between" style={{ marginTop: 10 }}>
                <span className="btn sm in">{role}</span>
                {ev.creator?.id === meId && (
                  <button className="btn sm" onClick={() => handleCancel(ev.id)} disabled={isPending}>Cancel</button>
                )}
              </div>
            </div>
          </div>
