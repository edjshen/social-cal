...
export async function acceptRequest(connId: string) {
  const uid = await requireUserId();
  const c = (await getDb().select().from(connections).where(eq(connections.id, connId)).limit(1))[0];
...
export async function setTier(otherId: string, tier: 'inner' | 'orbit') {
  ...
  if (existing) await getDb().update(placements).set({ tier }).where(eq(placements.id, existing.id));
  else await getDb().insert(placements).values({ id: crypto.randomUUID(), ownerId: uid, otherId, tier });
  revalidatePath('/circles');
}
