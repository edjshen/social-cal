...
export function enrich(ev: BarycalEvent, viewer: string | null, ctx: EnrichCtx, opts: { detail?: boolean } = {}) {
  ...
  const going = att.filter((a) => ATTEND.includes(a.rsvp) && mineIds.has(a.userId) && notGhost(a.userId));
  const out: any = {
  ...
  if (opts.detail) {
    ...
    const shown = ev.visibility === 'public'
      ? goingAll
      : goingAll.filter((a) => a.userId === ev.creatorId || a.userId === viewer || mineIds.has(a.userId));
    out.attendees = shown.slice(0, 12).map((a) => ({ ...publicUser(byId(ctx.users, a.userId)), rsvp: a.rsvp }));
  }
  return out;
}
