...
export async function calendarWindow(meId: string, startISO: string, endISO: string, opts?: { includePastRecurring?: boolean }) {
  ...
  const all = await (opts?.includePastRecurring ? getCalendarEventsBetween(startISO, endISO) : getEventsBetween(startISO, endISO));
  return all
