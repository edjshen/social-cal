...
test('parses a leading # and a full URL', () => {
  ...
  assert.deepEqual(parseFragment(`https://plur.nyc/rooms#${frag}`), {
    id: 'AAAA',
    k: 'BBBB',
    v: 1,
    event: false,
...
test('buildRoomUrl shape', () => {
  const url = buildRoomUrl('AAAA', 'BBBB', 'https://plur.nyc');
  assert.equal(url, 'https://plur.nyc/rooms#i=AAAA&k=BBBB&v=1');
  // Trailing slash on origin is tolerated.
  assert.equal(buildRoomUrl('AAAA', 'BBBB', 'https://plur.nyc/'), url);
});
