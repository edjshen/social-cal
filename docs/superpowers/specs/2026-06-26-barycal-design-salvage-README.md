...
prototype's code was kept. What survives here is the *visual + motion design
thinking*, which is stack-agnostic and worth drawing on if barycal's UI is ever
...
- `2026-06-26-barycal-barycenter-elevated-direction.md` — the authoritative
  ...
  laws, the ownable barycenter-physics signature (the social graph *is* the
...
- The prototype's `src/motion/` capability-tier hooks (`useWebGL`,
  ...
  included. Reimplement the *pattern* for Next.js if wanted.
