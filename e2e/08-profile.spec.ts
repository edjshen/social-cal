...
test.describe('Profile (You) Page', () => {
  ...
  test('shows share link or public profile link', async ({ page }) => {
    ...
    const shareLink = page.locator('button:has-text("Share"), a:has-text("Share"), button[aria-label*="share" i], [class*="share"]');
  ...
  test('can edit profile/bio', async ({ page }) => {
    const editBtn = page.locator('button:has-text("Edit"), button[aria-label*="edit" i], [class*="edit"]').first();
  ...
  test('bio and scenes are displayed', async ({ page }) => {
    ...
    if (!hasBio) {
      const bodyText = await page.locator('body').textContent() || '';
      const hasBioText = bodyText.toLowerCase().includes('bio') || bodyText.toLowerCase().includes('scene');
      if (!hasBioText) {
  ...
  test('visibility settings are accessible from profile', async ({ page }) => {
    const visibilityEl = page.locator('[class*="visibility"], button:has-text("Visibility"), [data-visibility]');
