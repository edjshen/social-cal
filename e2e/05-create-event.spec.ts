...
test.describe('Create Event Flow', () => {
  ...
  test('clicking create button opens create sheet/modal', async ({ page }) => {
    ...
    const sheet = page.locator('[class*="Sheet"], [class*="sheet"], [class*="CreateSheet"], [role="dialog"], [class*="modal"]');
  ...
  test('create sheet has event type options', async ({ page }) => {
    ...
    const typeOptions = page.locator('button:has-text("Plan"), button:has-text("Event"), button:has-text("Intention"), [class*="type"]');
  ...
  test('can fill in and submit a plan', async ({ page }) => {
    ...
    // Fill in a title
    const titleInput = page.locator('input[name="title"], input[placeholder*="title" i], input[placeholder*="name" i], textarea[name="title"]').first();
  ...
  test('create sheet can be dismissed/closed', async ({ page }) => {
    ...
    const closeBtn = page.locator('button[aria-label*="close" i], button[aria-label*="dismiss" i], button:has-text("×"), button:has-text("✕")');
  ...
  test('visibility tier options are available in create form', async ({ page }) => {
    ...
    const visibilityEl = page.locator('[class*="visibility"], select[name="visibility"], button:has-text("Inner"), button:has-text("Orbit"), button:has-text("Public")');
